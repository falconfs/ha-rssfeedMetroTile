...
export const generateMockFeedItems = (count: number): FeedItem[] => {
  const titles = [
...
export const generateLongTitleItems = (count: number): FeedItem[] => {
  const longTitles = [
...
export const generateNoDescriptionItems = (count: number): FeedItem[] => {
...
export const generateOldDateItems = (count: number): FeedItem[] => {
  return generateMockFeedItems(count).map((item, i) => {
...
export const generateMixedImageItems = (count: number): FeedItem[] => {
  return generateMockFeedItems(count).map((item, i) => {
...
export const generateBrokenImageItems = (count: number): FeedItem[] => {
  return generateMockFeedItems(count).map(item => ({
    ...item,
    links: [
      { type: 'text/html', href: item.link },
...
export const generateSlowLoadingImageItems = (count: number): FeedItem[] => {
  return generateMockFeedItems(count).map((item, i) => ({
    ...item,
    links: [
      { type: 'text/html', href: item.link },
...
export const createMockHass = (feedItems: FeedItem[]) => {
  return {
    states: {
      'sensor.die_zeit': {
      ...
      'sensor.malformed_data': {
        ...
        state: 'ok',
        attributes: {
          entries: 'not-an-array' as any, // Wrong type
...
export const presetConfigs: Record<string, any> = {
  default: {
...
// HA Theme Presets
export const HA_THEMES: Record<string, Record<string, string>> = {
  'default-light': {
...
// Grid Presets
export const GRID_PRESETS: Record<string, { columns: number; rows: number }> = {
