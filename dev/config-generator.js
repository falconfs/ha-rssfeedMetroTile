 ...
 *
 * This module automatically generates UI controls based on configuration schema
...
/**
 * Control Definition Interface (converted to JSDoc)
 ...
 * @property {*} [dependencyValue]
 */

/**
 * Extract possible values from configuration constants
 ...
 * @returns {Map<string, ControlDefinition[]>}
 */
 ...
 * @param {ControlDefinition} control
 * @returns {string}
 */
export function generateControlHTML(control) {
 ...
 * @returns {string}
 */
export function generateAllControlsHTML() {
 ...
 * @returns {boolean}
 */
export function isControlVisible(controlId, currentConfig) {
 ...
 * @param {Record<string, any>} currentConfig
 */
export function updateControlVisibility(currentConfig) {
  CONFIG_SCHEMA.forEach(control => {
    if (!control.dependency) return;

    const element = document.querySelector(`[data-control="${control.id}"]`);
    if (!element) return;
